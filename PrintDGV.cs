...
namespace PrintDataGrid
{
    class PrintDGV
    {
        ...
        private static DateTime _tarih;

        public static void Print_DataGridView(DataGridView dgv1, string prntTitle,DateTime tarih)
        {
            ...
            try
            {
                ...
                PrintTitle = prntTitle;
                PrintAllRows = true;
                FitToPageWidth = false;
                List<string> strList = new List<string>();
                strList.Add("baslik");
                strList.Add("bilgi");
                SelectedColumns = strList;
                ...
                if (PageNo == 1)
                {
                    foreach (DataGridViewColumn GridCol in dgv.Columns)
                    {
                        ...
                        if (FitToPageWidth)
                            ...
                            if(GridCol.HeaderText == "baslik")
                                tmpWidth = GridCol.Width + 40;
                            else
                                tmpWidth = (int)(Math.Floor((double)((double)GridCol.Width /
                                           (double)TotalWidth * (double)TotalWidth *
                                           ((double)e.MarginBounds.Width / (double)TotalWidth)))); ;
                ...
                while (RowPos <= dgv.Rows.Count - 1)
                {
                    ...
                    else
                    {
                        if (NewPage)
                        {
                            ...
                            //Draw LOGO
                            // Create image.
                            Image newImage = Image.FromFile("Logoo.gif");
                            ...
                            String s = _tarih.ToString("dd.MM.yyyy");
                        ...
                        foreach (DataGridViewCell Cel in GridRow.Cells)
                        {
                            ...
                            if(Cel.Value.ToString() == "")
                            {
                               // e.Graphics.DrawRectangle(Pens.Black, new Rectangle((int)ColumnLefts[i],
                                      //  tmpTop, (int)ColumnWidths[i], 1));
                                Pen p = new Pen(Color.Black);
                                e.Graphics.DrawLine(p, (int)ColumnLefts[i], tmpTop, (int)ColumnLefts[i]+(int)ColumnWidths[i], tmpTop);
                            }
                            i++;
                        }
                        tmpTop += CellHeight;
